...
export const adminService = {
  ...
  /**
   * Upload document with realtime progress via Server-Sent Events (SSE)
   * Streams progress updates from Python backend during upload and indexing
   */
  async uploadDocumentStream(params: {
    file: File;
    document_name: string;
    school_name: string;
    standard: string;
    ...
    const { file, document_name, school_name, standard, subject, onProgress } = params;
    ...
    return new Promise((resolve, reject) => {
      ...
      // Use fetch for SSE streaming (axios doesn't support SSE well)
      const pythonUrl = import.meta.env.VITE_PYTHON_URL || "http://localhost:8000";
      
      fetch(`${pythonUrl}/upload-stream`, {
        method: "POST",
        body: formData,
      })
        .then(async (response) => {
          ...
          while (true) {
            ...
            if (done) {
              ...
              if (lastEvent?.status === "success") {
                resolve({
                  success: true,
                  document_id: lastEvent?.data?.document_id,
                });
              } else if (lastEvent?.status === "error") {
                reject(new Error(lastEvent?.message || "Upload failed"));
              } else {
